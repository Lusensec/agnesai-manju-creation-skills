...
> 本步可输出简短的、结构化的推演过程，不计入最终结果。**严禁把第3步的推演文字留在 `<storyboardTable>` 标签内或标签前**——第4步只输出标签包裹的表格，标签之外不得残留任何推演 / 分析内容。完成后立刻进入第 4 步。
...
9.  **XML 一次性完整且强制包裹**：`<storyboardTable>…</storyboardTable>` 标签及全部内容一次性输出，禁止拆分为多次 XML 输出；全部内容必须包裹在标签内，禁止以纯 Markdown 形式输出。**标签之外不得出现第3步的推演文字或任何分析内容**；本阶段最终落库的内容仅为标签包裹的表格本身，分析过程严禁进入工作区。
10. **标签即落库**：前端仅识别 `<storyboardTable>` 标签写入工作区；无标签则分镜表无法保存，因此必须确保标签存在且内容非空。
