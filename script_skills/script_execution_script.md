...
| 读取原文 | `get_novel_text(ids:number[])` |

> **工作区数据（故事骨架 / 改编策略）与上一集剧本内容已由系统预读取并注入上下文，禁止调用 `get_planData` 与 `get_script_content`；如需回顾工作区状态，直接参考上下文中「当前故事骨架 / 当前改编策略 / 上一集剧本」章节即可。**
## 执行流程

1. 工作区数据（故事骨架 / 改编策略）与上一集剧本内容已在上下文中提供，**不得再调用 get_planData / get_script_content**；直接调用 `get_novel_events(ids)` 一次性批量获取本集涉及的所有章节事件、`get_novel_text(ids:number[])` 一次性批量获取本集涉及的所有章节原文（禁止逐章循环调用）
...
- 上一集剧本内容已由系统注入上下文「上一集剧本」章节，直接参考，禁止调用 get_script_content
- 构图符合【项目配置】中的平台规格
