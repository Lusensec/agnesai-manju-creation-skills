...
处理：逐集编写，每次调用执行层处理一集
输出：SQLite 中的剧本记录
工具：get_novel_events(ids:number[]) + get_planData + get_novel_text(ids:number[]) → insert_script_to_sqlite（所有章节读取必须一次性批量，禁止逐章循环）
前置条件：阶段2（改编策略）通过审核
```
...
> **⚠️ 强制派发规则**：当用户明确要求「重新执行 / 重做 / 重新生成某阶段」（如"重新执行阶段2：改编策略""重做故事骨架""重新写剧本"），决策层**必须立即调用对应的 `run_sub_agent_*` 工具**真实执行任务，绝不允许只回显项目配置、只复述任务描述、或仅口头告知"已为你重新执行"。只要指令中出现明确的阶段名 / 任务名，就直接 dispatch，不要等待用户二次确认或自行补全内容。
...
| "重新执行 / 重做 / 重新生成 阶段N" / "重新执行改编策略" / "重新执行故事骨架" / "重新写剧本" | **立即直接调用对应 `run_sub_agent_*`（如 `run_sub_agent_adaptationStrategy`），附带【项目配置】+ 任务指令；禁止只回显项目配置、只复述任务、只口头告知"已为你重新执行"，必须真正发起工具调用** |
